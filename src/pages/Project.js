...
const Project = () => {
  ...
  useEffect(() => {
    const timeout = setTimeout(() => {
      ...
      `);
      socket?.emit("send-code", id, {html : html ? html :"",css: css ? css :"",js : js ? js : ""});
    }, 300);
    ...
    try {
        ...
        if (data.status != 200) {
          setError(data.message);
            ...
            console.log(data.message);
            setContributers(prevContributers => [...prevContributers,data.newContributer]);
        }
      } catch (err) {
        console.error(err);
        setError("Oups something Went Wrong :(");
      ...
      {error ? (
        ...
        <div> 
            <section className="flex justify-center items-center mb-4">
              {contributers.map(contributer => (
                  <div key={contributer._id} className="m-2 text-center">
                      <div className="rounded-full bg-gray-900 text-center text-white p-2 h-10 w-10">{contributer.username[0].toUpperCase()}</div>
                      <span className="opacity-75">{contributer.username.slice(0,8)}</span>
                    </div>
              ))}
          </section>
          ...
          <header className="flex items-center justify-between">
            ...
            {project.userId === user._id && <button onClick={addContributer} className="px-5 py-3 cursor-pointer bg-gray-900 text-white rounded hover:opacity-80">share</button>}
            <button onClick={saveProject} className="px-5 py-3 cursor-pointer bg-green-600 text-white rounded hover:opacity-80">save</button>
            </div>
          </header>
